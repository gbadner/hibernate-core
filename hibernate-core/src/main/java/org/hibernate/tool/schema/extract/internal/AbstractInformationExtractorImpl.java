...
public abstract class AbstractInformationExtractorImpl implements InformationExtractor {
	...
	protected String determineCatalogFilter(Identifier catalog) throws SQLException {
	...
	protected String determineSchemaFilter(Identifier schema) throws SQLException {
	...
	protected abstract ResultSet getCatalogsResultSet() throws SQLException;
	protected abstract String getCatalogColumnName();

	@Override
	public boolean catalogExists(Identifier catalog) {
		try {
			final ResultSet resultSet = getCatalogsResultSet();

			try {
				while ( resultSet.next() ) {
					final String existingCatalogName = resultSet.getString( getCatalogColumnName() );
	...
	protected abstract ResultSet getSchemasResultSet(String catalogFilter, String schemaFilter) throws SQLException ;
	...
	public boolean schemaExists(Identifier catalog, Identifier schema) {
		try {
			...
			final ResultSet resultSet = getSchemasResultSet( catalogFilter, schemaFilter );
	...
	protected abstract String getTablesResultSetTableTypeColumn();

	protected abstract String getTablesResultSetRemarksColumn();
	...
	private TableInformation extractTableInformation(ResultSet resultSet) throws SQLException {
		...
		final TableInformationImpl tableInformation = new TableInformationImpl(
				...
				identifierHelper(),
				tableName,
				isPhysicalTableType( resultSet.getString( getTablesResultSetTableTypeColumn() ) ),
				resultSet.getString( getTablesResultSetRemarksColumn() )
		);
	...
	public TableInformation getTable(Identifier catalog, Identifier schema, Identifier tableName) {
		if ( catalog != null || schema != null ) {
			...
			try {
				...
				final ResultSet resultSet = getTablesResultSet(
						null,
						null,
						tableNameFilter,
						tableTypes
	...
	public NameSpaceTablesInformation getTables(Identifier catalog, Identifier schema) {
		...
		try {
			ResultSet resultSet = getTablesResultSet(
					catalogFilter,
					schemaFilter,
					"%",
					tableTypes
	...
	protected abstract ResultSet getColumnsResultSet(
			String catalogFilter,
			String schemaFilter,
			String tableNamePattern,
			String columnNamePattern) throws SQLException;

	protected abstract String getColumnsResultSetTableNameColumn();
	protected abstract String getColumnsResultSetColumnNameColumn();
	protected abstract String getColumnsResultSetDataTypeColumn();
	protected abstract String getColumnsResultSetTypeNameColumn();
	protected abstract String getColumnsResultSetColumnSizeColumn();
	protected abstract String getColumnsResultSetDecimalDigitsColumn();
	protected abstract String getColumnsResultSetIsNullableColumn();
	...
	private void populateTablesWithColumns(
		...
		try {
			ResultSet resultSet = getColumnsResultSet( catalogFilter, schemaFilter, null, "%" );
			try {
				String currentTableName = "";
				TableInformation currentTable = null;
				while ( resultSet.next() ) {
					if ( !currentTableName.equals( resultSet.getString( getColumnsResultSetTableNameColumn() ) ) ) {
						currentTableName = resultSet.getString( getColumnsResultSetTableNameColumn() );
					...
					if ( currentTable != null ) {
						final ColumnInformationImpl columnInformation = new ColumnInformationImpl(
								currentTable,
								DatabaseIdentifier.toIdentifier( resultSet.getString( getColumnsResultSetColumnNameColumn() ) ),
								resultSet.getInt( getColumnsResultSetDataTypeColumn() ),
								new StringTokenizer( resultSet.getString( getColumnsResultSetTypeNameColumn() ), "() " ).nextToken(),
								resultSet.getInt( getColumnsResultSetColumnSizeColumn() ),
								resultSet.getInt( getColumnsResultSetDecimalDigitsColumn() ),
								interpretTruthValue( resultSet.getString( getColumnsResultSetIsNullableColumn() ) )
						);
	...
	protected abstract ResultSet getTablesResultSet(
			String catalogFilter,
			String schemaFilter,
	...
	private TableInformation locateTableInNamespace(
		...
		try {
			ResultSet resultSet = getTablesResultSet( catalogFilter, schemaFilter, tableNameFilter, tableTypes );
	...
	protected abstract String getTablesResultSetTableNameColumn();
	...
	private TableInformation processTableResults(
		...
		try {
			...
			while ( resultSet.next() ) {
				if ( tableName.equals( Identifier.toIdentifier( resultSet.getString( getTablesResultSetTableNameColumn() ),
																tableName.isQuoted() ) ) ) {
					if ( found ) {
	...
	protected abstract String getTablesResultSetTableTypesTableConstant();

	protected boolean isPhysicalTableType(String tableType) {
		if ( extraPhysicalTableTypes == null ) {
			return getTablesResultSetTableTypesTableConstant().equalsIgnoreCase( tableType );
		}
		else {
			if ( getTablesResultSetTableTypesTableConstant().equalsIgnoreCase( tableType ) ) {
				return true;
			}
	...
	private void addColumns(TableInformation tableInformation) {
		...
		try {
			ResultSet resultSet = getColumnsResultSet(
					catalogFilter,
					schemaFilter,
			...
			try {
				while ( resultSet.next() ) {
					final String columnName = resultSet.getString( getColumnsResultSetColumnNameColumn() );
					final ColumnInformationImpl columnInformation = new ColumnInformationImpl(
							tableInformation,
	...
	protected abstract ResultSet getPrimaryKeysResultSet(
			String catalogFilter,
			String schemaFilter,
			Identifier tableName) throws SQLException;

	protected abstract String getPrimaryKeysResultSetPrimaryKeyNameColumn();
	protected abstract String getPrimaryKeysResultSetColumnPositionColumn();
	protected abstract String getPrimaryKeysResultSetColumnNameColumn();
	...
	public PrimaryKeyInformation getPrimaryKey(TableInformationImpl tableInformation) {
		...
		try {
			ResultSet resultSet =
					getPrimaryKeysResultSet( catalogFilter, schemaFilter, tableInformation.getName().getTableName() );
			...
			try {
				while ( resultSet.next() ) {
					final String currentPkName = resultSet.getString( getPrimaryKeysResultSetPrimaryKeyNameColumn() );
					...
					final int columnPosition = resultSet.getInt( getPrimaryKeysResultSetColumnPositionColumn() );

					final Identifier columnIdentifier = DatabaseIdentifier.toIdentifier(
							resultSet.getString( getPrimaryKeysResultSetColumnNameColumn() )
					);
	...
	public Iterable<IndexInformation> getIndexes(TableInformation tableInformation) {
		...
		try {
			ResultSet resultSet = getIndexInfoResultSet(
					catalogFilter,
					schemaFilter,
					tableName.getTableName().getText(),
