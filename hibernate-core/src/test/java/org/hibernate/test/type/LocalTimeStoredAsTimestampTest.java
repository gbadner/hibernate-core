...
public class LocalTimeStoredAsTimestampTest extends BaseCoreFunctionalTestCase {
	...
	public LocalTimeStoredAsTimestampTest(
	...
	protected void configure(Configuration cfg) {
		super.configure( cfg );
		cfg.setProperty( AvailableSettings.DIALECT, H2DialectOverrideSqlCodeTimeWithTimestamp.class.getName() );
	}

	private LocalTime getExpectedLocalTime() {
	...
	protected Class<?>[] getAnnotatedClasses() {
		return new Class[] { EntityWithLocalTime.class };
	}
	...
	public void writeThenRead() {
		withDefaultTimeZone( defaultTimeZone, () -> {
			inTransaction( session -> {
				session.persist( new EntityWithLocalTime( 1, getExpectedLocalTime() ) );
			} );
			inTransaction( session -> {
				LocalTime read = session.find( EntityWithLocalTime.class, 1 ).value;
				assertEquals(
						"Writing then reading a value should return the original value",
						getExpectedLocalTime(), read
				);
			} );
		} );
	}
	...
	private static final class EntityWithLocalTime {
		@Id
		@Column(name = "theid")
		private Integer id;
	...
	public static class H2DialectOverrideSqlCodeTimeWithTimestamp extends H2Dialect {

		@Override
		protected SqlTypeDescriptor getSqlTypeDescriptorOverride(int sqlCode) {
			if ( sqlCode == Types.TIME ) {
